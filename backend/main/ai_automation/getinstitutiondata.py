...
class GetInstitutionData(APIView):
    def get(self, request, pk):
        ...
        try:
            ...
            # Decode JWT manually to check expiry
            try:
        ...
        try:
            data = supabase.table('question').select('*').eq("uid", pk).execute()

            if not data.data:
                return Response({'message': 'No institution data found'}, status=status.HTTP_404_NOT_FOUND)
